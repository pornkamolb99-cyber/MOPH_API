...
SQL_CANCER_LINK_DIAGNOSIS_IPD = """
SELECT ADMMASTER.HN,
       ADMMASTER.AN,
       IpdDiag.icd10,
       CASE
            WHEN IpdDiag.diagtype = '1' THEN '1'
            WHEN IpdDiag.diagtype = '2' THEN '3'
            WHEN IpdDiag.diagtype = '3' THEN '4'
            WHEN IpdDiag.diagtype = '4' THEN '2'
            ELSE NULL
       END AS diag_type
FROM ADMMASTER
JOIN IpdDiag
...
def fetch_rows(target_date: date) -> list[dict[str, Any]]:
    with connect() as conn:
        cursor = conn.cursor()
        cursor.execute(SQL_CANCER_LINK_DIAGNOSIS_IPD, target_date.isoformat())
...
def run(target_date: date) -> tuple[int, int, int]:
    ...
    for an, payload in patient_group.items():
        result = client.send_cancer_link_diagnosis_ipd(payload)
        ...
        if is_success(result.ok, success):
            ...
            fail_count += 1
            print(
                "CANCER_LINK_DIAGNOSIS_IPD FAIL",
                an,
                result.status_code,
                result.text[:500],
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="ส่งข้อมูล Cancer Link Diagnosis IPD")
    ...
    if args.dry_run:
        print(
            f"CANCER_LINK_DIAGNOSIS_IPD วันที่ {target_date} "
            f"พบข้อมูล {len(patient_group)} AN / {len(rows)} diagnosis rows"
        )
